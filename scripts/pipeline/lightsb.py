...
from scripts.pipeline import autoencoderkl

from src.light_sb import LightSB # make sure to add models/LightSB to path!!
...
class LightSB_BDD:
    def __init__(self, config: CN, np_data):
        ...
        # only night -> day
        self.X_train = torch.tensor(np_data["train_night"])
        self.Y_train = torch.tensor(np_data["train_day"])
        self.X_test = torch.tensor(np_data["val_night"])
        self.Y_test = torch.tensor(np_data["val_day"])

        self.X_sampler = TensorSampler(self.X_train, device="cpu")
        self.Y_sampler = TensorSampler(self.Y_train, device="cpu")
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.config.D_LR)
    ...
    @staticmethod
    def update_config(cfg, args: argparse.Namespace):
        cfg.defrost()
        if hasattr(args, "DIM"):
            cfg.DIM = args.DIM
        ...
        if hasattr(args,"D_LR"):
            cfg.MODEL.D_LR = args.D_LR
        if hasattr(args, "N_POTENTIALS"):
            cfg.MODEL.N_POTENTIALS = args.N_POTENTIALS
        if hasattr(args, "IS_DIAGONAL"):
            cfg.MODEL.IS_DIAGONAL = args.IS_DIAGONAL
